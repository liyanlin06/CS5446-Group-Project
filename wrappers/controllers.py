...
class SimpleFactoryDiscreteController(Controller):
    def __init__(self, env_cfg) -> None:
        ...
        self.water_dim_high = self.water_dims
        self.no_op_dim_high = self.water_dim_high + self.no_op_dims

        self.total_act_dims = self.no_op_dim_high
        action_space = spaces.Discrete(self.total_act_dims)
        super().__init__(action_space)
    
    def _is_water_action(self, id):
        return id < self.water_dim_high

    def _get_water_action(self, id):
        return 2
    ...
    def action_to_lux_action(
        ...
        for k, factory in factories.items():
            ...
            action_queue = []
            no_op = False
            if self._is_water_action(choice):
                action_queue = self._get_water_action(choice)
            else:
                # action is a no_op, so we don't update the action queue
                no_op = True

            if not no_op:
                lux_action[factory["unit_id"]] = action_queue
    ...
    def action_masks(self, agent: str, obs: Dict[str, Any]):
        action_mask = np.ones((self.total_act_dims), dtype=bool)
        return action_mask
...
class SimpleUnitDiscreteController(Controller):
    ...
    def _get_transfer_action(self, id):
        ...
        transfer_resource = id // 5
        transfer_dir = id % 5
        return np.array([1, transfer_dir, transfer_resource, self.env_cfg.max_transfer_amount, 0, 1])
